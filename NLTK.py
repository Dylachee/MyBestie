...
train_data = train_data[:200]  # Ограничение до 1000 строк

# Загрузка тестовых данных
test_data = load_data('test_lr.csv')

# Извлечение слов и их меток из данных
train_words = train_data['Word'].tolist()[:200]  # Используйте только первые 1000 слов
train_labels = train_data['Tag'].tolist()[:200]  # Используйте только первые 1000 меток
...
train_df = pd.DataFrame({'Word': train_tokens[:200], 'Morphemes': train_morphemes[:200]})
